...
_FRAME_SIZE = 224
_QUEUE_SIZE = 20
#Don't use a number higher than 1 until multithreading testing is fixed!
...
def main(dataset, mode, variant, split):
    print(mode)
    assert mode in ['rgb', 'flow', 'mixed']
    log_dir = os.path.join(_LOG_ROOT, 'test-%s-%s-%s-%d' % (dataset, mode,variant, split))
    if not os.path.exists(log_dir):
        os.mkdir(log_dir)

    logging.basicConfig(level=logging.INFO, filename=os.path.join(
        log_dir, 'log-%s-%s-%d' % (mode, variant, split)+'.txt'), filemode='w', format='%(message)s')
    ...
    _DATA_ROOT = {
    'ucf101': {
        'rgb': 'data/preprocessed-'+variant+'/test',
        'flow': 'data/preprocessed-'+variant+'/test'
    }}
    
    _, test_info_rgb, class_num, _ = load_info(
        dataset, root=_DATA_ROOT[dataset], mode='rgb', variant=variant, split=split)
    _, test_info_flow, _, _ = load_info(
        dataset, root=_DATA_ROOT[dataset], mode='flow', variant=variant, split=split)
    ...
    for i in range(video_size):
        print('Real Label:',test_info_rgb[i]['label'])
        if mode in ['rgb', 'mixed']:
...
#        print('input_label.shape:', input_label.shape)
        ...
        tmp = np.sum(top_1)
        true_count += tmp
        print('Video %d: %s, accuracy: %.4f (%d/%d) , name: %s' %
              (i+1, tmp==1, true_count/video_size, true_count, video_size, video_name))
        logging.info('Video%d: %s, accuracy: %.4f (%d/%d) , name:%s' %
                     (i+1, tmp==1, true_count/video_size, true_count, video_size, video_name))
