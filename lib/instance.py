...
class Video(object):
    ...
    def load_img(self, index):
        ...
        if self.tag == 'rgb':
            ...
            u_img = Image.open(os.path.join(img_dir.format('x'), self.img_format.format(index, ''))).convert('L')
            v_img = Image.open(os.path.join(img_dir.format('y'), self.img_format.format(index, ''))).convert('L')
            return  [u_img, v_img]
