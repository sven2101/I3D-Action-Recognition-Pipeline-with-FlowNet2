...
def load_info(name, root, task='train', mode='rgb', variant='flownet2', split=1):
    if name in ['ucf101', 'hmdb51']:
        train_info, val_info = get_info(name, root, mode, variant, split)
        return train_info, val_info, _CLASS_NUM[name], _IMG_FORMAT[name]
    else:
        raise ValueError('name of dataset must be ucf101 or hmdb51')


def get_info(name, root, mode='rgb',variant='flownet2', split=1):
    base_dir = root
    data_dir = 'data/'+name+'/'+mode+'_'+variant+'.txt'
