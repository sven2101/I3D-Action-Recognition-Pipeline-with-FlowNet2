...
def main(dataset='ucf101', mode='rgb',variant='flownet2',split=1): 
    assert mode in ['rgb', 'flow'], 'Only RGB data and flow data is supported'
    
    file_check = True
    print('Checking Files...')
    if mode == 'rgb':
        file_check = check_files(os.path.join('./data', dataset, mode+'_'+variant+'.txt'),"")
    elif mode == 'flow':
        file_check = check_files(os.path.join('./data', dataset, mode+'_'+variant+'.txt'),"x")
        if file_check == True:
            file_check = check_files(os.path.join('./data', dataset, mode+'_'+variant+'.txt'),"y")
    
    if file_check == False:
        print('Frames are not equal - Please fix before training!')
        return
    print('Files are okay! - Continue!')
   
    log_dir = os.path.join(_LOG_ROOT, 'finetune-%s-%s-%s-%d' %
                           (dataset, mode, variant, split))
    ...
    train_info, test_info = split_data(
        os.path.join('./data', dataset, mode+'_'+variant+'.txt'),
    ...
    print('----Here we start!----')
    print('Output writes to ' + log_dir)
...
#check files if the actually amount of images is the same as in the .txt files
#if not fix before start training and try again    
def check_files(file_to_check,formatter):
    check = True
    file_list = open(file_to_check, "r")
    for f in file_list:
        splitted_file = f.split(" ")
        supposed_length = int(splitted_file[2])
        path = splitted_file[1].format(formatter)
        files = [f2 for f2 in listdir(path) if isfile(join(path, f2))]
        real_length = 0
        for max_file in files:
            if int(max_file[5:-4])>real_length:
                real_length = int(max_file[5:-4])
        if supposed_length != real_length:
            check = False
            print("Length is different:",path,supposed_length,real_length)
    return check
...
if __name__ == '__main__':
    ...
    p.add_argument('mode', type=str, help="type of data, e.g., rgb or flow")
